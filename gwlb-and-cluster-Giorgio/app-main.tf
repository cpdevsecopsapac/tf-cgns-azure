...
resource "azurerm_container_group" "container-app-juiceshop" {
  ...
  resource_group_name = azurerm_resource_group.rg-app-A.name
  os_type             = "Linux"
  ip_address_type     = "Private" # one of [Public Private None]
#  network_profile_id  = azurerm_network_profile.profile-app-juiceshop.id
  subnet_ids          = ["${azurerm_subnet.net-spoke-0-web.id}"]
...
#  depends_on = [azurerm_lb.lb-app-B]
#}
resource "azurerm_network_interface_backend_address_pool_association" "lb-backend-pool-B-nic" {
  network_interface_id    = azurerm_network_interface.nic-vmspoke.id
  ip_configuration_name   = "${var.app-name-vm}-eth0-config"
  backend_address_pool_id = azurerm_lb_backend_address_pool.lb-backend-pool-B.id

  depends_on = [azurerm_network_interface.nic-vmspoke,azurerm_lb_backend_address_pool.lb-backend-pool-B]
}

# for the purpose of explicit outbound connection
resource "azurerm_lb_outbound_rule" "example" {
  ...
  depends_on = [azurerm_lb_backend_address_pool.lb-backend-pool-B,azurerm_lb.lb-app-B]
}
...
output "webapp-A-pub-IP-fqdn" {
  value = "${azurerm_public_ip.publicip-app-A.fqdn}"
}
output "webapp-B-pub-IP-fqdn" {
  value = "${azurerm_public_ip.publicip-app-B.fqdn}"
}
output "webapp-C-pub-IP-fqdn" {
  value = "${azurerm_public_ip.publicip-app-C.fqdn}"
}
